...
def mclust_R(adata, num_cluster, modelNames='EEE', used_obsm='profile_gene_img', random_seed=1998):
    ...
    try:
        ...
        # Add a safety check for data dimensions
        data = adata.obsm[used_obsm]
        print(f"Data shape for mclust: {data.shape}, requested clusters: {num_cluster}")
        
        if data.shape[0] < num_cluster or data.shape[1] < 1:
            raise ValueError(f"Invalid data shape: {data.shape} for clustering with {num_cluster} clusters")
        ...
        print(f"mclust_R error: {str(e)}")
        raise RuntimeError(f"Error during Mclust clustering: {str(e)}. Please adjust the number of principal components and try again.")
...
def clustering(adata, n_clusters=7, radius=10, refinement=False, seed=1998, 
    ...
    # Apply safety checks to avoid dimensionality issues
    max_gene_pcs = min(adata.obsm['rec_gene'].shape[0] - 1, adata.obsm['rec_gene'].shape[1])
    gene_pcs = min(gene_pcs, max_gene_pcs)
    print(f"Using gene_pcs={gene_pcs} (constrained by data dimensions)")
    ...
    if gene_variance is not None:
        ...
        gene_pcs = min(gene_pcs, max_gene_pcs)  # Apply safety constraint
    ...
    # Apply safety check for image PCs too
    max_img_pcs = min(rec_img.shape[0] - 1, rec_img.shape[1])
    image_pcs = min(image_pcs, max_img_pcs)
    ...
    if img_variance is not None:
        ...
        image_pcs = min(image_pcs, max_img_pcs)  # Apply safety constraint
    ...
    try:
        print("Attempting to use mclust_R for clustering...")
        adata = mclust_R(adata, used_obsm='profile_gene_img', num_cluster=n_clusters, random_seed=seed)
    except Exception as e:
        print(f"Error during Mclust clustering: {str(e)}")
        raise RuntimeError(f"Error during Mclust clustering: {str(e)}. Please adjust the number of principal components and try again.")
